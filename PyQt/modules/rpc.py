...
class MRPC:
    ...
    def callPresence(self, queue):
        #Я так сделал блять и оно работает, каким хуем оно работает тебя ебать не должно, я сам бля не ебу, если надо - смотри ебучий код
        switch = 0
        lasttrack = 0
        song_time_switch = 0
        song_time_check = 0
                        ...
                        if song_time_check != song_time[0]:
                            if song_time_switch == 1:
                                ...
                                if text_switch == 1:
                                    ...
                                    self.updatePresence(artist, song, image_link, song_link,song_time=song_time[1] - song_time[0], time_switch=time_switch)
